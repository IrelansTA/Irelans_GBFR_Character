...
CBUFFER_START(UnityPerMaterial)
    ...
    half _SpecularRampWidth;

    half _OutlineNoiseContrast;
...
half4 SampleMixMap(float2 uv)
{
    half4 mixColor = half4(1, 1, 1, 1);
    mixColor = SAMPLE_TEXTURE2D(_MixMap, sampler_MixMap, uv);

    // mixColor.r *= (1 - _Smoothness) ;

    // mixColor.g *= (_Metallic );
    return mixColor;
}
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    //采样mix贴图
    half4 mixColor = SampleMixMap(uv);
    outSurfaceData.metallic =mixColor.g;
    outSurfaceData.specular = half3(0.0, 0.0, 0.0);
    outSurfaceData.smoothness = 1- mixColor.r;
    outSurfaceData.occlusion = mixColor.z;
    //应用Hatching贴图到材质各通道中
    half hatching = mixColor.a;
    outSurfaceData.albedo *= hatching;
    // outSurfaceData.metallic *= hatching;
    // outSurfaceData.smoothness *= hatching;
    outSurfaceData.occlusion *= hatching;
