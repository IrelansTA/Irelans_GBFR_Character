...
half clearCoatMask, bool specularHighlightsOff)
{
    ...
    half3 brdf = brdfData.diffuse;
        ...
        {
            half BRDFSpecular = DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS, viewDirectionWS);
            // brdf += brdfData.specular * BRDFSpecular;//原始Brdf高光颜色

            //GFBR 彩色高光 start

                //计算采样Ramp图的UV坐标
                half ramp_width = saturate(_SpecularRampWidth);
                half rampU = saturate(BRDFSpecular*brdfData.specular);
                rampU = ramp_width * rampU  - ramp_width+1;//采样范围收敛，使u轴采样结果往右侧(u=1)收敛
                half2 rampUV = half2(saturate(rampU), 0.5);

                //采样Ramp图
                half3 rampColor = SAMPLE_TEXTURE2D(_RampMap, sampler_RampMap, rampUV).rgb;

                //结合BRDFSpecular和Ramp图的颜色
                half3 GFBR_BRDFSpecular = BRDFSpecular * rampColor ;
                brdf += brdfData.specular * GFBR_BRDFSpecular;
